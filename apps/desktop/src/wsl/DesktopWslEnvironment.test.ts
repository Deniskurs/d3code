...
describe("WSL runtime cache", () => {
  ...
  it("installs through a temporary directory and only reuses valid completed caches", () => {
    const script = buildWslRuntimeInstallScript(
      "/mnt/c/Program Files/D3 Code/wsl-runtime.tar.gz",
      "1.2.3-x64",
      "b".repeat(64),
    );

    expect(script).toContain('runtime_parent="$HOME/.d3/wsl-runtime"');
    ...
    expect(script).toContain(
      "tar -xzf '/mnt/c/Program Files/D3 Code/wsl-runtime.tar.gz' -C \"$runtime_tmp\" --strip-components=1",
    );
  ...
  it("verifies the archive digest before extracting, and only on a cache miss", () => {
    ...
    expect(script).toContain(
      "archive_sha=$(sha256sum '/mnt/c/Program Files/D3 Code/wsl-runtime.tar.gz' | cut -d ' ' -f 1)",
    );
  ...
  it("parses only absolute Linux runtime paths", () => {
    expect(parseWslRuntimeRoot("runtimeRoot:/home/josh/.d3/wsl-runtime/1.2.3-x64\n")).toBe(
      "/home/josh/.d3/wsl-runtime/1.2.3-x64",
    );
  ...
  it("invalidates a cache by dropping its ready marker, not the tree", () => {
    ...
    expect(script).toContain('rm -f "$HOME/.d3/wsl-runtime/1.2.3_x64/.t3code-wsl-runtime-ready"');
...
describe.skipIf(posixShellRunner === null)("WSL runtime install script (executed)", () => {
  ...
  const createFixture = () => {
    ...
    return {
      ...
      archiveSha,
      runtimeId,
      runtimeParent: `${work}/home/.d3/wsl-runtime`,
      runtimeRoot: `${work}/home/.d3/wsl-runtime/${runtimeId}`,
      serverEntry: `${work}/home/.d3/wsl-runtime/${runtimeId}/t3`,
      installScript,
  ...
  it("prunes old and markerless caches without touching retained, active, locked, or unrelated roots", () => {
    const result = runShell(
      [
        ...
        "work=$(mktemp -d)",
        'home="$work/home"',
        'runtime_parent="$home/.d3/wsl-runtime"',
