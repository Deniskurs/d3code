  ...
  it.effect("uses canonical source-tree icons for unpackaged development", () =>
    Effect.gen(function* () {
      ...
      const fileSystemLayer = FileSystem.layerNoop({
        exists: (path) => Effect.succeed(String(path).includes("/assets/devis/dev/")),
      });
      ...
      assert.match(Option.getOrThrow(icons.ico), /assets\/devis\/dev\/icon\.ico$/);
      assert.match(Option.getOrThrow(icons.png), /assets\/devis\/dev\/icon\.png$/);
