...
describe("DesktopEnvironment", () => {
  it.effect("ignores the official app home override and keeps D3 data isolated", () =>
    Effect.gen(function* () {
      const environment = yield* makeEnvironment({}, { T3CODE_HOME: "/Users/alice/.t3" });
      assert.equal(environment.stateDir, "/Users/alice/.d3/userdata");
      assert.equal(environment.userDataDirName, "d3code");
      assert.equal(environment.legacyUserDataDirName, "D3 Code (Alpha)");
    }),
  );
  ...
  it.effect("derives state paths and development identity inside Effect", () =>
    Effect.gen(function* () {
      ...
      assert.equal(environment.appUserModelId, "com.deniskurs.d3code.dev");
      assert.equal(environment.linuxWmClass, "d3code-dev");
      assert.equal(environment.linuxDesktopEntryName, "com.deniskurs.D3Code.Development.desktop");
      assert.deepEqual(
  ...
  it.effect("uses the stable desktop entry as the packaged Linux portal identity", () =>
    Effect.gen(function* () {
      ...
      assert.equal(environment.linuxDesktopEntryName, "com.deniskurs.D3Code.desktop");
    }),
  );
  ...
  it.effect("keeps implicit development state separate from production state", () =>
    Effect.gen(function* () {
      ...
      assert.equal(development.stateDir, "/Users/alice/.d3/dev");
      assert.equal(production.stateDir, "/Users/alice/.d3/userdata");
    }),
  );
  ...
  it.effect("uses a configured app user model id override", () =>
    Effect.gen(function* () {
      const environment = yield* makeEnvironment(
        {},
        {
          T3CODE_DESKTOP_APP_USER_MODEL_ID: " com.deniskurs.d3code.dev.local ",
          VITE_DEV_SERVER_URL: "http://localhost:5173",
        },
      );

      assert.equal(environment.appUserModelId, "com.deniskurs.d3code.dev.local");
    }),
  );
