...
describe("AcpSessionRuntime", () => {
  ...
  for (const initialToolUpdate of [false, true]) {
    it.effect(
      `segments assistant text when a tool begins with ${initialToolUpdate ? "tool_call_update" : "tool_call"}`,
      () =>
        Effect.gen(function* () {
          const runtime = yield* AcpSessionRuntime.AcpSessionRuntime;
          const notes: Array<AcpSessionRuntime.AcpSessionRuntimeEvent> = [];
          yield* runtime.getEvents().pipe(
            Stream.runForEach((event) => {
              if (event._tag === "EventStreamBarrier") {
                return Deferred.succeed(event.acknowledge, undefined);
            ...
            AcpSessionRuntime.layer({
              spawn: {
                ...
                env: {
                  ...
                  T3_ACP_INITIAL_TOOL_UPDATE: initialToolUpdate ? "1" : "0",
                },
              },
              cwd: process.cwd(),
              clientInfo: { name: "t3-test", version: "0.0.0" },
