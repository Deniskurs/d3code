...
export class DesktopAppUnreachableError extends Schema.TaggedError<DesktopAppUnreachableError>()(
  ...
  override get message(): string {
    return "Could not reach the D3 Code desktop app. Start or update the desktop app on this machine, then run `t3 app` again. A running D3 Code server is not enough.";
  }
}
...
export class DesktopAppRequestFailedError extends Schema.TaggedError<DesktopAppRequestFailedError>()(
  ...
  override get message(): string {
    return `D3 Code could not open ${this.workspaceRoot} (${this.code}).`;
  }
}
...
const appEnvironment = Config.all({
  t3Home: Config.string("D3CODE_HOME").pipe(Config.option, Config.map(Option.getOrUndefined)),
...
const runAppCommand = Effect.fn("cli.app")(function* (flags: {
  ...
  yield* Console.log(`Opened ${workspaceRoot} in D3 Code.`);
});
...
export const appCommand = Command.make("app", {
  ...
  Command.withDescription("Open a project in the running D3 Code desktop app."),
