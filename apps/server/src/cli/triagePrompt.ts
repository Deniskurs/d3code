...
export const TRIAGE_PLAYBOOK = `# D3 Code triage playbook

You are a support engineer for D3 Code (https://github.com/pingdotgg/t3code), working
...
- How is D3 Code running on this machine: \`npx t3 serve\` in a terminal, the
...
Do not patch the D3 Code source as a fix. A good issue with strong repro steps
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your D3 Code triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A D3 Code user is \
...
export const buildTriageContext = (input: TriageContextInput) => `# D3 Code triage context
