...
describe("t3 app", () => {
  ...
  it.effect("uses D3CODE_HOME or --base-dir and sends the default or explicit path", () =>
    withTempDirectory("t3-app-command-test-", (root) =>
      Effect.gen(function* () {
        ...
        yield* runCli(["app"], { D3CODE_HOME: baseDir });
  ...
  it.effect("never searches a dev state directory for an explicit T3 home", () =>
    withTempDirectory("t3-app-explicit-test-", (root) =>
      Effect.gen(function* () {
        ...
        const envError = yield* runCli(["app"], { D3CODE_HOME: baseDir }).pipe(Effect.flip);
