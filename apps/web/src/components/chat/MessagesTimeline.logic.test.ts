...
describe("deriveMessagesTimelineRows", () => {
  ...
  it("shows the worktree setup card instead of the working placeholder", () => {
    ...
    expect(finishedRows.map((row) => row.kind)).toEqual(["message", "working", "waiting"]);
  });
  ...
  it("keeps subagent spawn rows outside turn folds even after they settle", () => {
    ...
    expect(activeRows(direct, new Set(["agent-b"]))).not.toContain("waiting");
    expect(activeRows(workflow, new Set(["wf-1"]))).not.toContain("waiting");
    expect(activeRows(direct, new Set())).toContain("waiting");
    expect(activeRows(direct, new Set(["agent-b"]), "turn-2")).toContain("waiting");
    ...
    for (const [toolLifecycleStatus, tone, sourceActivityKind] of [
      ...
      for (const trailingEntries of [[], [laterTool]]) {
        ...
        const active = trailingEntries.length === 0 || toolLifecycleStatus === "inProgress";
        expect(rows.filter((row) => row.kind === "work-live")).toMatchObject([
          {
            id: active ? "live-activity-row" : "work-live:spawn-entry",
            entry: { id: trailingEntries.length ? "later-tool" : "spawn-entry" },
            active,
          },
        ]);
        expect(rows.some((row) => row.kind === "work")).toBe(false);
        expect(rows.some((row) => row.kind === "waiting")).toBe(!active);
      }
    }
