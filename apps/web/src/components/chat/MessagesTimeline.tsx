...
interface TimelineRowSharedState {
  ...
  onSteerQueuedMessage: ((id: string) => void) | null;
  onRemoveQueuedMessage: ((id: string) => void) | null;
  onPauseQueuedMessage: ((id: string, paused: boolean) => void) | null;
}
...
interface MessagesTimelineProps {
  ...
  onPauseQueuedMessage?: (id: string, paused: boolean) => void;
}
...
export const MessagesTimeline = memo(function MessagesTimeline({
  ...
  const sharedState = useMemo<TimelineRowSharedState>(
    () => ({
      ...
      onSteerQueuedMessage: onSteerQueuedMessage ?? null,
      onRemoveQueuedMessage: onRemoveQueuedMessage ?? null,
      onPauseQueuedMessage: onPauseQueuedMessage ?? null,
    }),
    [
...
const TimelineRowContent = memo(function TimelineRowContent({ row }: { row: TimelineRow }) {
  ...
  return (
    ...
    >
      ...
      {row.kind === "waiting" ? <WaitingTimelineRow /> : null}
...
function QueuedMessageTimelineRow({
  ...
  const statusLabel =
    status === "submitted"
      ...
      : status === "sending"
        ? "Preparing or sending this saved message"
        : status === "failed"
    ...
    <div className="flex flex-col items-end" data-queued-message-id={queuedMessage.id}>
      <div className="max-w-[80%] rounded-2xl border border-dashed border-border p-3 text-message-foreground/80">
        ...
        {queuedMessage.error ? (
          <div
            role="alert"
            className="mt-1.5 whitespace-pre-wrap break-words text-xs text-destructive"
          >
            {queuedMessage.error}
          </div>
        ) : null}
              ...
              <TooltipTrigger
                render={
                  <Button
                    ...
                    disabled={
                      inFlight || queuedMessage.sendNow === true || !ctx.onSteerQueuedMessage
                    }
                ...
                render={
                  <Button
                    ...
                    disabled={
                      inFlight ||
                      status === "failed" ||
                      !!queuedMessage.dispatchAttempted ||
                      !ctx.onPauseQueuedMessage
                    }
                    aria-label={
                      queuedMessage.holdUntilUserAction
                        ? "Resume queued message"
                        : "Pause queued message"
                    }
                  />
                }
              >
              ...
              <TooltipTrigger
                render={
                  <Button
                    ...
                    disabled={!canRestore || !ctx.onRemoveQueuedMessage}
...
function WaitingTimelineRow() {
  const { isCompacting, isPreparingWorktree } = use(TimelineRowActivityCtx);
  // Reserve the activity row during setup so the handoff keeps the same height.
  return (
    <div className="min-h-7">
      {isPreparingWorktree || isCompacting ? null : (
        <LiveActivityRow label="Waiting for response" iconName="message-circle" active shimmer />
      )}
    </div>
  );
}
...
function LiveWorkEntryTimelineRow({ row }: { row: Extract<TimelineRow, { kind: "work-live" }> }) {
  ...
  const preview = row.active && !row.expanded ? liveReasoningPreview(row.entry) : undefined;
  ...
  return (
    <div className="min-w-0">
      ...
      {preview ? (
        <p className="ml-8 mt-1 line-clamp-3 whitespace-pre-wrap break-words text-xs leading-relaxed text-muted-foreground select-text">
          {preview}
        </p>
      ) : null}
    </div>
