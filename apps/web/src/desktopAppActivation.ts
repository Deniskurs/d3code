  ...
  if (projectId === null) {
    try {
      ...
      return failure(
        ...
        errorMessage(error, "D3 Code could not add the project."),
      );
    }
  }
  ...
  try {
    ...
    if (opened === null) {
      return failure(
        request.requestId,
        "thread-open-failed",
        "D3 Code could not open a new thread for the project.",
      );
    }
    return {
      version: 1,
    ...
    return failure(
      request.requestId,
      "thread-open-failed",
      errorMessage(error, "D3 Code could not open a new thread for the project."),
    );
  }
}
