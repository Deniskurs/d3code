...
describe("brand-assets", () => {
  ...
  it("keeps development, nightly, and production icon families separate", () => {
    expect([
      ...
      "assets/devis/dev/app-icon.icon",
      "assets/devis/nightly/app-icon.icon",
      "assets/devis/prod/app-icon.icon",
    ]);
    expect(BRAND_ASSET_PATHS.developmentDesktopIconPng).toMatch(/^assets\/devis\/dev\//);
    expect(BRAND_ASSET_PATHS.nightlyMacIconPng).toMatch(/^assets\/devis\/nightly\//);
    expect(BRAND_ASSET_PATHS.productionMacIconPng).toMatch(/^assets\/devis\/prod\//);
  });
});
